from models import Monomial, Normal, Favard, ChebII, Bern
...
def build_optimizer(args, model):
    if args.basis not in  ['Favard', 'Favard2']:
...
def build_model(args,edge_index,norm_A):
    ...
    if args.basis == 'Favard2':
        model = Favard_2(args.basis, edge_index, norm_A, n_channel=3, K=10)
...
def main(args):
    ...
    for idx in tqdm(range(0, n_samples)):
        print(data[idx]['true_filter'])
        signal, filtered_signal_ground = data[idx]['signal'].float().to(args.gpu), data[idx]['filtered_signal'].float().to(args.gpu)
    ...
    _mean = np.array(loss_samples).mean()
    _std = np.array(loss_samples).std()
    print("Model: {}, Mean: {}, Std: {}".format(args.basis, _mean, _std))
    return _mean, _std
...
if __name__ == '__main__':
    ...
    '''
    results = []
    for basis in ['Normal','ChebII','Bern','Favard','Monomial']:
        args.basis = basis
        args.n_epochs = 500
        _mean, _std = main(args)
        results.append((basis, _mean, _std))
    for _ in results:
        print(_)
